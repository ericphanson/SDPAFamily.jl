...
SDPA-GMP.jl uses `Float64` for all problem data and solutions by default. To use, for example, `BigFloat` instead, simply call `SDPA_GMP.Optimizer{BigFloat}()`. Note that with `MathOptInterface` all the problem data must be of the same type, i.e. `BigFloat` in this case. 
