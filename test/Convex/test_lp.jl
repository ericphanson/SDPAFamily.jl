@testset "LP Atoms: " for solver in solvers
    @testset "abs atom" begin
        x = Variable()
        p = Problem{BigFloat}(:minimize, abs(x), x<=-1)
        ...
        x = Variable(2,2)
        p = Problem{BigFloat}(:minimize, sum(abs(x)), x[2,2]>=1, x[1,1]>=1, x>=0)
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
    ...
    @testset "maximum atom" begin
        ...
        p = Problem{BigFloat}(:minimize, maximum(x), x >= a)
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
    ...
    @testset "minimum atom" begin
        ...
        p = Problem{BigFloat}(:maximize, minimum(x), x <= a)
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
        ...
        p = Problem{BigFloat}(:maximize, objective, constraints)
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
    ...
    @testset "max atom" begin
        ...
        p = Problem{BigFloat}(:minimize, maximum(max(x, y)), [x >= a, y >= b])
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
        max_a = maximum(a)
        max_b = maximum(b)
    ...
    @testset "min atom" begin
        ...
        a = reshape(shuffle(collect(0.01:0.01:1.0)), (10, 10))
        b = reshape(shuffle(collect(0.01:0.01:1.0)), (10, 10))
        p = Problem{BigFloat}(:maximize, minimum(min(x, y)), [x <= a, y <= b])
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
        min_a = minimum(a)
        min_b = minimum(b)
    ...
    @testset "pos atom" begin
        x = Variable(3)
        a = [-2; 1; 2]
        p = Problem{BigFloat}(:minimize, sum(pos(x)), [x >= a, x <= 2])
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
    ...
    @testset "sumlargest atom" begin
        x = Variable(2)
        p = Problem{BigFloat}(:minimize, sumlargest(x, 2), x >= [1; 1])
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
        @test p.optval ≈ 2 atol=TOL
        ...
        x = Variable(4, 4)
        p = Problem{BigFloat}(:minimize, sumlargest(x, 3), x >= eye(4), x[1, 1] >= 1.5, x[2, 3] >= 2.1)
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
    ...
    @testset "sumsmallest atom" begin
        ...
        x = Variable(3, 2)
        p = Problem{BigFloat}(:maximize, sumsmallest(x, 3), x >= 2, x <= 5, sumlargest(x, 3) <= 12)
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
    ...
    @testset "dotsort atom" begin
        ...
        x = Variable(2, 2)
        p = Problem{BigFloat}(:minimize, dotsort(x, [1 2; 3 4]), sum(x) >= 7, x >= 0, x <= 2, x[2, 2] <= 1)
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
        @test p.optval ≈ 19 atol=TOL
        @test evaluate(dotsort(x, [1, 2, 3, 4])) ≈ 19 atol=TOL
    ...
    @testset "norm inf atom" begin
        x = Variable(3)
        p = Problem{BigFloat}(:minimize, norm_inf(x), [-2 <= x, x <= 1])
        @test vexity(p) == ConvexVexity()
        solve!(p, solver())
