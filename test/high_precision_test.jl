...
@testset "High precision test" begin
    ...
    time = @elapsed solve!(p, SDPAFamily.Optimizer(silent = true, presolve = true, variant = :plain))
    ...
    time = @elapsed solve!(p, SDPAFamily.Optimizer(silent = true, presolve = true, variant = :dd))
    @test p.optval ≈ (6 - sqrt(big"2.0"))/4 atol=1e-15
    error = abs(p.optval - (6 - sqrt(big"2.0"))/4)
    @info "SDPA-dd solved the problem with an absolute error of " error time

    time = @elapsed solve!(p, SDPAFamily.Optimizer(silent = true, presolve = true, variant = :qd))
    error = abs(p.optval - (6 - sqrt(big"2.0"))/4)
    @test p.optval ≈ (6 - sqrt(big"2.0"))/4 atol=1e-15
    @info "SDPA-qd solved the problem with an absolute error of " error time

    time = @elapsed solve!(p, SDPAFamily.Optimizer(silent = true, presolve = true, variant = :gmp))
