...
@testset "High precision test" begin
    ...
    time = @elapsed solve!(p, SDPA_GMP.Optimizer(silent = false, presolve = true, variant = :dd))
    ...
    time = @elapsed solve!(p, SDPA_GMP.Optimizer(silent = false, presolve = true, variant = :qd))
    error = abs(p.optval - (6 - sqrt(big"2.0"))/4)
    @test p.optval ≈ (6 - sqrt(big"2.0"))/4 atol=1e-17
    @info "SDPA-qd solved the problem with an absolute error of " error time

    time = @elapsed solve!(p, SDPA_GMP.Optimizer(silent = false, presolve = true, variant = :gmp))
