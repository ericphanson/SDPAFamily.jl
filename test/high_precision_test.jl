...
@testset "High precision test" begin
    ...
    solve!(p, SDPA_GMP.Optimizer(silent = true, presolve = true, variant = :dd))
    @test p.optval ≈ (6 - sqrt(big"2.0"))/4 atol=1e-10

    solve!(p, SDPA_GMP.Optimizer(silent = true, presolve = true, variant = :qd))
    @test p.optval ≈ (6 - sqrt(big"2.0"))/4 atol=1e-10

    solve!(p, SDPA_GMP.Optimizer(silent = true, presolve = true, variant = :gmp))
